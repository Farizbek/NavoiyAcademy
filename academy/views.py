...
def checktasks(request):
    ...
    content = {'lessons' : query_lesson, 'tasks' : query, 'title' : 'Home works!'}
    return render(request, 'academy/tasks.html', content)

def student(request):
    query   = StudentModel.objects.all().select_related('lesson')
    content = {'student' : query, 'title' : "O'quvchilar baholari"}
    return render(request, 'academy/student.html', content)